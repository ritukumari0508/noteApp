...
const DeleteListModal: FC<DeleteListModalProps> = ({ listId }) => {
  ...
  const deleteListHandler = () => {
    ...
    if(list) {
      dispatch(setNotification(`Note "${list.name}" deleted!`, 'danger'));
    }
  }
    ...
    <div className="modal is-active">
      ...
      <div className="modal-card">
        <header className="modal-card-head has-text-centered">
          <p className="modal-card-title">Are you sure you want to delete this note ?</p>
        </header>
        <div className="modal-card-body">
          <h2 className="is-size-5 has-text-centered">All tasks related to this note will be deleted</h2>
          <div className="content">
            {list?.tasks.length === 0 ?
              <p className="has-text-centered pt-4 mb-0">No tasks in this note!</p>
              :
              <ul>
                {list?.tasks.map(task => (
